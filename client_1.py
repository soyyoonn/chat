...
class WindowClass(QMainWindow, form_class) :
    ...
    def chat_page(self):
        ...
        self.client_socket.send((f'{self.item.text()}004+001+{self.username.text()}011').encode())  # 004: 채팅방이름, 001 : 이전채팅, 011: 채팅방알림(닉네임) 전송
    ...
    def send_chat(self):
        ...
        message=(f'{roomname}004+{senders_name}:{data}002+0')# 채팅방이름, 닉네임, 송신메시지 전송준비(0은 전송후 스플릿할 때 리스트 길이 맞춰주려고 넣은것)
        ...
        while True:
            try:
                ...
                if recv_data[-3:] == '002':
                    ...
                    if self.roomname_check == self.item.text():     # 채팅시 서버로 전송한 채팅방이름과 현재 접속한 채팅방이름이 같으면
                        for i in j_data2:
                            self.receivemessage.addItem(f'[{i[0]}:{i[1]}]\n{i[2]}')
                    else:
                        pass
